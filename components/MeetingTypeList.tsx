...
const MeetingTypeList = () => {
    ...
    const createMeeting = async () => {
       ...
       try{
        if(!values.dateTime) {
            toast({ title: "Please sellect a date and time",})
            return;
        }
        ...
        {!callDetails ? (<MeetingModal
            ...
            handleClick={createMeeting}>
                ...
                <div className=' flex flex-col gap-2.5'>
                    <label className='text-base text-normal leading-[22px] text-rose-2 '>
                        Add a description
                    </label>
                   <Textarea onChange={(e) => { setValues({...values, description: e.target.value})}} className=' border-none bg-dark-2 focus-visible:ring-0 focus-visible:ring-offset-0' />
                </div>
